...
import { normalizeId } from '~hooks/useVerification'
 ...
 *
 * The election ID is not asked for — `/votes/{id}` resolves it — because a
 * second field is exactly where a nervous voter gives up.
 */
export const VerifyHero = () => {
  const navigate = useNavigate()
  const [voteId, setVoteId] = useState('')
  // `/verify/{voteId}` is the canonical, explorer.vote-compatible short form:
  // one identifier in the URL, election resolved on arrival.
  const go = () => {
