...
export const useColorMode = (): {
  ...
  const setColorMode = useCallback((value: ColorMode) => setTheme(value), [setTheme])

  return { colorMode, toggleColorMode, setColorMode }
}

/**
 * Returns `light` until the component has mounted, so the server/first paint and
 * the hydrated tree agree even though the resolved theme is only known client-side.
 */
